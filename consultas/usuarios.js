...
function getClientIp(req) {
  ...
  usuarioRouter.post("/login", async (req, res, next) => {
    try {
      ...
      const bloqueoQuery = 'SELECT * FROM tblipbloqueados WHERE clienteId = ?';
      const [bloqueos]= await req.db.query(bloqueoQuery, [clientId]);
      ...
      await req.db.query("DELETE FROM tblipbloqueados  clienteId = ? ", [clientId]);
